...
| --- | --- | --- |
| [frontend](https://github.com/mohamed1abdullah/Production-Grade_GitOps-Driven_Microservices-Demo/blob/main/src/frontend) | Go | Exposes an HTTP server to serve the website. Does not require signup/login and generates session IDs for all users automatically. |
| [cartservice](https://github.com/mohamed1abdullah/Production-Grade_GitOps-Driven_Microservices-Demo/blob/main/src/cartservice) | C# | Stores the items in the user's shopping cart in Redis and retrieves it. |
| [productcatalogservice](https://github.com/mohamed1abdullah/Production-Grade_GitOps-Driven_Microservices-Demo/blob/main/src/productcatalogservice) | Go | Provides the list of products from a JSON file and ability to search products and get individual products. |
| [currencyservice](https://github.com/mohamed1abdullah/Production-Grade_GitOps-Driven_Microservices-Demo/blob/main/src/currencyservice) | Node.js | Converts one money amount to another currency. Uses real values fetched from European Central Bank. It's the highest QPS service. |
| [paymentservice](https://github.com/mohamed1abdullah/Production-Grade_GitOps-Driven_Microservices-Demo/blob/main/src/paymentservice) | Node.js | Charges the given credit card info (mock) with the given amount and returns a transaction ID. |
| [shippingservice](https://github.com/mohamed1abdullah/Production-Grade_GitOps-Driven_Microservices-Demo/blob/main/src/shippingservice) | Go | Gives shipping cost estimates based on the shopping cart. Ships items to the given address (mock) |
| [emailservice](https://github.com/mohamed1abdullah/Production-Grade_GitOps-Driven_Microservices-Demo/blob/main/src/emailservice) | Python | Sends users an order confirmation email (mock). |
| [checkoutservice](https://github.com/mohamed1abdullah/Production-Grade_GitOps-Driven_Microservices-Demo/blob/main/src/checkoutservice) | Go | Retrieves user cart, prepares order and orchestrates the payment, shipping and the email notification. |
| [recommendationservice](https://github.com/mohamed1abdullah/Production-Grade_GitOps-Driven_Microservices-Demo/blob/main/src/recommendationservice) | Python | Recommends other products based on what's given in the cart. |
| [adservice](https://github.com/mohamed1abdullah/Production-Grade_GitOps-Driven_Microservices-Demo/blob/main/src/adservice) | Java | Provides text ads based on given context words. |
| [loadgenerator](https://github.com/mohamed1abdullah/Production-Grade_GitOps-Driven_Microservices-Demo/blob/main/src/loadgenerator) | Python/Locust | Continuously sends requests imitating realistic user shopping flows to the frontend. |
...
```bash
https://github.com/mohamed1abdullah/Production-Grade_GitOps-Driven_Microservices-Demo.git
```
...
3. Install the AWS Load Balancer Controller.
    ...
    - `-set vpcId=vpc-xxxxxxxx`
        ...
        Replace `*cluster-name*` with the name of your cluster. In the following command, `aws-load-balancer-controller` is the Kubernetes service account that you created in a previous step.
...
app-alb-gateway   aws-alb-gateway-class   k8s-default-appalbga-7ce4f1fc4a-722129577.us-east-1.elb.amazonaws.com   True         43h
```
...
```bash
https://argocd.mohamedabdullah.site
```
...
jobs:
  build:
    ...
    steps:
      ...
      - name: Build Image
        run: |
              SERVICE="./src/${{ inputs.service }}"

              if [ -f "$SERVICE/Dockerfile" ]; then
                DOCKERFILE="$SERVICE/Dockerfile"
                CONTEXT="$SERVICE"
              elif [ -f "$SERVICE/src/Dockerfile" ]; then
                DOCKERFILE="$SERVICE/src/Dockerfile"
                CONTEXT="$SERVICE/src"
              else
                echo "Dockerfile not found for ${{ inputs.service }}"
                exit 1
              fi
...
helmCharts:
  - name: boutique-app
    repo: oci://ghcr.io/mohamed1abdullah/onlineboutique
    version: 0.10.4
...
spec:
  ...
  source:
    repoURL: https://github.com/mohamed1abdullah/Production-Grade_GitOps-Driven_Microservices-Demo.git
    targetRevision: HEAD
    path: .
...
```bash
kubectl describe pod frontend-56c4f757fc-tcjbr -n boutique-app | grep "^
 *Image:"
    Image:          ghcr.io/mohamed1abdullah/microservices-demo/frontend:sha-d9637f11466e229aa7d47860204c576951edaf3a
```
...
spec:
  ...
  applicationRefs:
    - namePattern: "boutique-*"
      ...
      images:
        - alias: adservice
          imageName: ghcr.io/mohamed1abdullah/microservices-demo/adservice

        - alias: cartservice
          imageName: ghcr.io/mohamed1abdullah/microservices-demo/cartservice

        - alias: checkoutservice
          imageName: ghcr.io/mohamed1abdullah/microservices-demo/checkoutservice

        - alias: currencyservice
          imageName: ghcr.io/mohamed1abdullah/microservices-demo/currencyservice

        - alias: emailservice
          imageName: ghcr.io/mohamed1abdullah/microservices-demo/emailservice

        - alias: frontend
          imageName: ghcr.io/mohamed1abdullah/microservices-demo/frontend

        - alias: paymentservice
          imageName: ghcr.io/mohamed1abdullah/microservices-demo/paymentservice

        - alias: productcatalogservice
          imageName: ghcr.io/mohamed1abdullah/microservices-demo/productcatalogservice

        - alias: recommendationservice
          imageName: ghcr.io/mohamed1abdullah/microservices-demo/recommendationservice

        - alias: shippingservice
          imageName: ghcr.io/mohamed1abdullah/microservices-demo/shippingservice

        - alias: loadgenerator
          imageName: ghcr.io/mohamed1abdullah/microservices-demo/loadgenerator
```
...
Access the website `app.mohamedabdullah.site`
...
spec:
  hostnames:
    - "prometheus.mohamedabdullah.site"
  parentRefs:
  - group: gateway.networking.k8s.io
    namespace: default
    kind: Gateway
...
NAME               HOSTNAMES                   AGE
grafana-route      ["grafana.mohamedabdullah.site"]      30m
prometheus-route   ["prometheus.mohamedabdullah.site"]   41s
```
...
Create an IAM role and attach a policy. AWS maintains an AWS managed policy or you can create your own custom policy. You can create an IAM role and attach the AWS managed policy with the following command. Replace `*cluster-name*` with the name of your cluster. The command deploys an AWS CloudFormation stack that creates an IAM role and attaches the IAM policy to it.
...
aws eks create-addon \
  ...
  --service-account-role-arn arn:aws:iam::<AWS_ACCOUNT_ID>:role/eksctl-terraform-cluster-addon-iamserviceacco-Role1-BtKkwyzNLIlN \
...
helm install eck-operator elastic/eck-operator --version 3.4.1 -n logging
```
...
```bash
helm install eck-elasticsearch elastic/eck-elasticsearch --version 0.19.1 -n logging
```
...
```bash
helm show values elastic/eck-beats --version 0.19.1 > observability/helm-values/eck-beats-0.19.1.yaml
```
...
vi **`observability/helm-values/eck-beats-0.19.1.yaml`**
...
```bash
helm upgrade -i eck-beats elastic/eck-beats --version 0.19.1 -f helm-values/eck-beats-0.19.1.yaml -n logging
```
...
```bash
helm show values elastic/eck-kibana --version 0.19.1 > observability/helm-values/eck-kibana-0.19.1.yaml
```

Edit:

vi **`observability/helm-values/eck-kibana-0.19.1.yaml`**
...
```bash
helm install eck-kibana elastic/eck-kibana --version 0.19.1 -f observability/helm-values/eck-kibana-0.19.1.yaml -n logging
```
...
NAME           HOSTNAMES               AGE
kibana-route   ["kibana.mohamedabdullah.site"]   74s
```
...
```bash
docker tag us-central1-docker.pkg.dev/google-samples/microservices-demo/adservice:v0.10.4 ghcr.io/mohamed1abdullah/microservices-demo/adservice:v0.10.4
```

Push the image:

```bash
  docker push ghcr.io/mohamed1abdullah/microservices-demo/adservice:v0.10.4 
```
</details>
...
```
oci://ghcr.io/mohamed1abdullah/charts/onlineboutique
```
...
```bash
helm push onlineboutique-0.10.4.tgz oci://ghcr.io/mohamed1abdullah
```
...
```bash
helm install boutique oci://ghcr.io/mohamed1abdullah/onlineboutique --version 0.10.4
```
</details>
